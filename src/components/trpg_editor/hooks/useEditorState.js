...
const replaceActivePage = (items, activePageId, updates) => (
  items.map((page) => (page.id === activePageId ? { ...page, ...updates } : page))
);
...
export function useEditorState(scenario) {
  ...
  const updatePageContent = useCallback((newContent) => {
    setPages((currentPages) => replaceActivePage(currentPages, activePageIdRef.current, { content: newContent }));
  }, []);

  const updatePageTitle = useCallback((newTitle) => {
    setPages((currentPages) => replaceActivePage(currentPages, activePageIdRef.current, { title: newTitle }));
  }, []);
  ...
  const clearPage = useCallback(() => {
    setPages((currentPages) => replaceActivePage(currentPages, activePageIdRef.current, { content: '' }));
  }, []);
